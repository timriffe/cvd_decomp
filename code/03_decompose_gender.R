...
# IN <-read_csv("data/TP_final.csv.gz")
# IN |> 
#   select(period, gender, educ, transition, age, p = p_fit) |> 
...
# it seems I should derive these prevalences in the code using denom..
...
prev_for_kit <- tibble(educ = c("basic","secondary","tertiary"),
                       prev_men = c(.155,.469,.375),
                       prev_women = c(.079,.354,.567)) |> 
  mutate(prev_men = prev_men / sum(prev_men),
         prev_women = prev_women / sum(prev_women))
...
dec_total <-
  dec2 |> 
  filter(educ != "total") |> 
  left_join(dec_weights, by = join_by(period, educ)) |> 
  group_by(period, educ) |> 
  mutate(cc_total = (cc / sum(cc)) * rate_effect)

# Our complete decomposition, check sums
dec_total |> 
  group_by(period) |> 
  summarize(cc_total = sum(cc_total)) |> 
  left_join(kit |> group_by(period) |> 
              summarize(init = sum(structure_effect)), by = join_by(period)) |> 
  mutate(check = cc_total + init)


# compare with stationary difference:
round(total_stationary$HLE_women - total_stationary$HLE_men, 2)
